...
plot_irf <- function(df_irf, shock_name, response_labels = NULL, shock_labels = NULL) {
  ...
  p <- ggplot(df_plot, aes(x = horizon, y = pe)) +
    ...
    facet_wrap(~response, scales = "free_y", ncol = 4, nrow = 2, labeller = lab) +
    labs(x = "Horizonte", y = "Respuesta", title = shock_title, caption = "IC al 95% (Bootstrap)") +
